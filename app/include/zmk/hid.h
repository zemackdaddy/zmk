...
static const uint8_t zmk_hid_report_desc[] = {
    ...
    /* USAGE_PAGE (Led) */
    HID_GI_USAGE_PAGE, HID_USAGE_LED,
    /* USAGE_MINIMUM (Num Lock) */
    HID_LI_USAGE_MIN(1), HID_USAGE_LED_NUM_LOCK,
    /* USAGE_MAXIMUM (Kana) */
    HID_LI_USAGE_MAX(1), HID_USAGE_LED_KANA,
    /* REPORT_SIZE (1) */
    HID_GI_REPORT_SIZE, 0x01,
    /* REPORT_COUNT (5) */
    ...
    HID_LI_USAGE_MAX(1), ZMK_HID_KEYBOARD_NKRO_MAX_USAGE,
    /* REPORT_SIZE (8) */
    HID_GI_REPORT_SIZE, 0x01,
    /* REPORT_COUNT (6) */
    HID_GI_REPORT_COUNT, ZMK_HID_KEYBOARD_NKRO_MAX_USAGE + 1,
    /* INPUT (Data,Ary,Abs) */
    HID_MI_INPUT, 0x02,
    ...
    HID_GI_REPORT_COUNT, CONFIG_ZMK_HID_KEYBOARD_REPORT_SIZE,
    /* INPUT (Data,Ary,Abs) */
    HID_MI_INPUT, 0x00,
#else
#error "A proper HID report type must be selected"
    ...
    /* LOGICAL_MAXIMUM (0x00FF)  - little endian, and requires two bytes because logical max is
       signed */
    HID_GI_LOGICAL_MAX(2), 0xFF, 0x00, HID_LI_USAGE_MIN(1), 0x00,
    /* USAGE_MAXIMUM (0xFF) */
    HID_LI_USAGE_MAX(1), 0xFF,
    /* INPUT (Data,Ary,Abs) */
    /* REPORT_SIZE (8) */
