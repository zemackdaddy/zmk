...
static int led_pwm_blink(const struct device *dev, uint32_t led, uint32_t delay_on,
                         uint32_t delay_off) {
    ...
    return pwm_pin_set_usec(led_pwm->dev, led_pwm->channel, period_usec, pulse_usec,
                            led_pwm->flags);
}
...
static int led_pwm_set_brightness(const struct device *dev, uint32_t led, uint8_t value) {
    ...
    return pwm_pin_set_cycles(led_pwm->dev, led_pwm->channel, led_pwm->period, pulse,
                              led_pwm->flags);
}
...
static int led_pwm_pm_control(const struct device *dev, enum pm_device_action action) {
    const struct led_pwm_config *config = DEV_CFG(dev);
...
{                                                                                           \
        ...
        .flags = DT_PHA_OR(led_node_id, pwms, flags, PWM_POLARITY_NORMAL),                         \
    \
},

#define LED_PWM_DEVICE(id)                                                                         \
    \
static const struct led_pwm led_pwm_##id[] = {DT_INST_FOREACH_CHILD(id, LED_PWM)\
};                                                                                                 \
    \
static const struct led_pwm_config led_pwm_config_##id = {                                         \
        .num_leds = ARRAY_SIZE(led_pwm_##id), .led = led_pwm_##id,                                 \
    \
};                                                                                                 \
    \
DEVICE_DT_INST_DEFINE(id, &led_pwm_init, led_pwm_pm_control, NULL, &led_pwm_config_##id,           \
                      POST_KERNEL, CONFIG_LED_INIT_PRIORITY, &led_pwm_api);
